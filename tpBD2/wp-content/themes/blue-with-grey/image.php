...
<div class="main">
	<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		...
		<p><?php printf( __( '<a href="%1$s">%2$s</a>', 'blue_with_grey' ), get_permalink( $post->post_parent ), get_the_title( $post->post_parent ));?></p>
		<p><a href="<?php echo wp_get_attachment_url($post->ID); ?>"><?php echo wp_get_attachment_image( $post->ID, 'medium' ); ?></a></p>
		<?php if ( !empty($post->post_excerpt) ) the_excerpt(); ?>
		<?php wp_link_pages(array('before' => '<p><strong>'.__('Páginas', 'blue_with_grey').':</strong> ', 'after' => '</p>', 'next_or_number' => 'number')); ?>
		<p class="pagination">
